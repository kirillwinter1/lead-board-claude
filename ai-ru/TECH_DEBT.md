...
- [ ] Log time (F90, итерация 2) — при списании на задачу БЕЗ оценки (`remainingH == null`) модалка ставит Remaining `0m`, а backend всегда шлёт `adjustEstimate=new` → Jira-остаток становится 0 вместо «не задан». Значение видно и редактируемо до сабмита (не тихо). Фикс: опускать `newEstimate`/`adjustEstimate` когда исходный остаток был null (проброс признака null из фронта и в `MyWorklogService`/`JiraWriteService`).
...
- [ ] **Eligible-эпики зависят от категоризации статусов.** `PokerController.isEligibleForPoker` считает эпик доступным для покера, если его статус категоризуется как `NEW`/`REQUIREMENTS`/`PLANNED` (через `WorkflowConfigService.categorize`). Раньше был явный whitelist имён статусов («грязная оценка»/«rough estimation»/«в работе»). Для тенанта, где статус вроде «Rough estimation» категоризован иначе, такой эпик не появится в пикере. Компромисс осознанный (убрали хардкод статусов), но при жалобах — проверить `status_mappings` тенанта. Плюс фильтрация идёт in-memory после `findEpicsByTeam` (не в SQL) — приемлемо для текущих объёмов.
- [ ] **Переоценка COMPLETED-стори без UI.** `setFinalEstimate` теперь допускает статус COMPLETED (чтобы исправить ошибочный финал на уровне API), но в комнате нет кнопки «re-open / переоценить завершённую стори» — только через прямой WS/REST. Добавить UI-путь.
