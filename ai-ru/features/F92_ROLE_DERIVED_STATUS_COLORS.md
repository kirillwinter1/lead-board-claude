...
| Оттенок Review | ~~Насыщеннее роли~~ → **светлее роли** (свап 22.07: work — насыщенный цвет роли, review — светлый тон) |
...
| kind | Цвет |
|------|------|
| `WORK` (и NULL) | цвет роли как есть (насыщенный) — активная работа заметна |
| `REVIEW` | светлый тон роли: lighten(roleColor, 0.65) — ревью «прозрачнее» |
| `WAITING` | серый токен `#DFE1E6` |

(Изначально было наоборот; поменяно местами по живому фидбеку 22.07.)
