...
- Каждый сегмент: `StatusBadge status={...}` слева, длительность справа; текущий подсвечен
  (жирный + маркер «сейчас»). Вертикальный таймлайн-«конверт».
- Итоги внизу: **«Всего»** (вся история) и **«Без «{первый статус}»»** — всего за вычетом
  первого (стартового, обычно New/бэклог) сегмента = время «в работе». Вторая строка
  показывается при ≥2 сегментах.
- Формат длительности: ≥1д → «Nд», иначе ≥1ч → «Nч», иначе «<1ч».

## Согласованность с бейджем ([[F79]])
`StatusAgeService.daysInStatus` раньше искал переход со **строгим** `to_status == jira_issues.status`
и при рассогласовании имён (локализация: текущий «Запланировано» vs changelog «Planned»)
падал на `jira_created_at`, показывая полный возраст вместо времени в текущем статусе.
Теперь якорь = **последний переход** задачи (как и текущий сегмент F81) → бейдж и тултип
всегда согласованы.
